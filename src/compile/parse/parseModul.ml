...
let parse_import_path_and_first_import(l: Lexer.t)(start: Loc.pos)(next: Token.t): Ast.import_path * Ast.local_declare =
	let n_leading_dots, start_name =
		...
		| Token.Dot ->
			...
			let n_extra_dots, name = count @@ fun () ->
				match Lexer.next l with
				| Token.Dot -> None
				| Token.Name n -> Some(n)
				| x -> ParseU.unexpected start l x in
			(n_extra_dots + 1), name
		| Token.Name n ->
			0, n
		| x ->
	...
	let names, first_import =
		ArrayU.build_loop0_with_first start_name @@ fun () ->
...
let parse_imports(l: Lexer.t): Ast.imports array =
	ArrayU.build_loop @@ fun () ->
		...
		let imported_names, continue =
			ArrayU.build_loop0_with_first first_import @@ fun () ->
				let start, next = Lexer.pos_next l in
				match next with
...
let f(l: Lexer.t): Ast.modul =
	...
	let decls = ArrayU.build_fold (start, next) @@ fun (start, next) ->
		match next with
		| Token.EOF ->
			None, None
		| next ->
