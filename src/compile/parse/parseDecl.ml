let parse_param(p: Parser.t): Ast.local_declare option =
	...
	| Token.Name name ->
		 ...
		 Some (Ast.LocalDeclare(Parser.loc_from p start, name, typ))
	| t ->
...
let parse_signature(p: Parser.t): Ast.signature =
	...
	let params = U.build_array (fun () -> parse_param p) in
	(* let params = U.build_array (fun () ->
		U.op_if (not (ParseU.try_eat_indent p)) (fun () -> ParseLocalDeclare.f p)) in *)
